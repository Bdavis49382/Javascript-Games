...
function startGame() {
  ...
  const clock = setInterval(function () {
    //run these functions every 100 milliseconds
    buildgrid();
    updatePlayer(clock);
    drawActors();
  }, 100);
...
function handleInput(event) {
  if (event.code == "KeyW" && playerDirection.join() != [0, 10].join()) {
    ...
    event.code == "KeyD" &&
    playerDirection.join() != [-10, 0].join()
  ) {
    playerDirection = [10, 0];
  } else if (
    event.code == "KeyS" &&
    playerDirection.join() != [0, -10].join()
  ) {
    playerDirection = [0, 10];
...
function updatePlayer(clock) {
  ...
  playerPos = playerPos.map(
    (value, index) => (value += playerDirection[index])
  );
...
function drawActors() {
  ...
  tailPieces.forEach((tailPiece) =>
    context.fillRect(tailPiece[0], tailPiece[1], 10, 10)
  );
